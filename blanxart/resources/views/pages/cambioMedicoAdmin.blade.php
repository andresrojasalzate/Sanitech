...
@section('content')
    <main class="allform-container">
        <x-boton-atras :url="route('cambiarMedico')" />

        <section class="allform-container-title">
            <h2 class="regular">Sol·licitud de canvi de metge</h2>
            <p>Gestioni el canvi de metge</p>
        </section>
        ...
        <section>
            <div class="contenedorCambioMedicoAdmin">
                ...
                <div id="formularioCambioMedico">
                    <cambioMedico id="cambio-medico-form" :csrf-token="'{{ csrf_token() }}'"
                        :medicos='@json($medicos)' :id-paciente='@json($idPaciente)'
                        :errors='@json($errors->toArray())'></cambioMedico>
                </div>
            </div>
        </section>
    </main>
